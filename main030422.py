...
[v,t,cd_0, cd, d_inst, a_i, t_station]= compilevt(a, s_avg_ms, Ndoors, capacity, N_cars, W, m, w_a, tstep) #input a is a scalar, output a is a vector
    ...
    while j < len(t): #len(t)
        ...
        #tractive force terms
            ...
            #tractive force + (20*theta[j])
                ...
                ft_2[j]= 0.0005*M*907.185*(track_r + track_c + weather_r + bearing_r + slope_term[j])  #lb/ton -> N/kg -> N
                ...
                ft_3[j]= 0.0005*M*907.185*(track_r + track_c + weather_r + bearing_r + slope_term[j])  #lb/ton -> N/kg -> N
                ...
                ft_2[j]= 4.4482*W*(init_term[j] + slope_term[j])
                j+=1
                ...
                ft_2[j]= 4.4482*W*(init_term[j]+ slope_term[j]+ end_term[j])
...
while i< len(t) :
      if a_i[i]<0: #during deceleration
          ...
          P_re[i]= regen_eff[i]*P[i] # for positive P
      else:
          regen_eff[i]= 0
          P_re[i]= regen_eff[i]*P[i]
      i+=1
...
E_nonre=P*(tstep/(60**2)) #E=P*t instantaneous energy (without regeneration)
...
E_total_nonre= E_nonre.sum() # total energy (without regeneration)


#%%cumulative
E_cumulative_nonre= np.zeros(len(t)) #initalise
...
plt.plot (t, P, P_re)
plt.subplot(313)
plt.plot (t,E_cumulative_nonre, E_cumulative)
