...
#import track data
df = pd.read_excel("elevation.xlsx")  
...
bearing_r= 10 +0.1*abs(50-avg_UK_temp)
...
#     #define theta, then define tractive effort
#     while j < len(t): #len(t)
#         #find the distances in table it's between to at a given time (checking each)
#         if  df.loc[slope_i,'distance']<= cd[j] <= df.loc[slope_i +1,'distance']: #and v_kph[j]>0:
...
while slope_i < df.shape[0]-1: #number of rows in df
    ...
    while j < len(t): #len(t)         
        ...
        # #if theta is positive it made a contribution
        #     if theta[j]>0:
        #         grade_r = 20*theta[j]
        
        
        #a different if loop
        # #starting tractive effort, for each start from complete stop
        
        if  j==0 or (v_kph[j]>0 and v_kph[j-1]==0): #e.g. v=0.8, prev v=0 # initially we do not want j-1 term involved
            
                ft_2[j]= 4.4482*M*(track_r + track_c + weather_r + bearing_r)# + grade_r)     
                j+=1
            
        # #about to start moving but v=0
        # v_kph[j] 
        
        # #tractive effort for when completely stationary
        ...
        # #tractive effort for when in motion
        # elif 
        ...
        # # elif v_kph[j]==0 and v_kph[j-1]<1:# for the deceleration, last term has speed>0, then 0, we want all terms but div by 0 (last term)
        # #     init_term[j]= (0.6+(20/w_a) +(0.01*v_kph[j]/1.61) + (K*((v_kph[j]/1.61)**2)/(w_a*n)))
        # #     ft_2[j]= 4.4482*W*(init_term[j])
        # #     j+=1
                
            
        # #tractive force + grade_r=(20*theta[j])
    ...
    #next time step
        j+=1
    #check in next distance bracket
    slope_i += 1
