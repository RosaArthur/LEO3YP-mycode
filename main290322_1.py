...
while slope_i < df.shape[0]-1: #number of rows in df
    ...
    while j < len(t): #len(t)
        ...
        #tractive force + (20*theta[j])
            ...
            ft_2[j]= 0.0005*M*907.185*(track_r + track_c + weather_r + bearing_r)# + grade_r)  #lb/ton -> N/kg -> N
        ...
        elif v_kph[j]==0 and v_kph[j-1]==0 : # and v_kph[j+1]==0 and j<(len(t)-)  #if stationary, ft_2= 0, no tractive effort
...
g= plt.figure(1)
plt.subplot(311)
plt.plot (t, ft_2) #[0:len(theta)] #ft_2
plt.subplot(312)
plt.plot (t, slope_term) #P
# plt.subplot(313)
# plt.plot (t, v)
